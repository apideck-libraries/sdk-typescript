...
<!-- UsageSnippet language="typescript" operationID="hris.companiesAll2" method="get" path="/hris/companies" -->
```typescript
...
async function run() {
  const result = await apideck.hris.companies.all({
...
import { hrisCompaniesAll } from "@apideck/unify/funcs/hrisCompaniesAll.js";
...
async function run() {
  const res = await hrisCompaniesAll(apideck, {
    serviceId: "salesforce",
    passThrough: {
      "search": "San Francisco",
    },
  ...
  if (res.ok) {
    ...
    console.log("hrisCompaniesAll failed:", res.error);
  }
}
...
**Promise\<[operations.HrisCompaniesAll2Response](../../models/operations/hriscompaniesall2response.md)\>**
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("hrisCompaniesCreate failed:", res.error);
  }
}
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("hrisCompaniesGet failed:", res.error);
  }
}
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("hrisCompaniesUpdate failed:", res.error);
  }
}
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("hrisCompaniesDelete failed:", res.error);
  }
}
