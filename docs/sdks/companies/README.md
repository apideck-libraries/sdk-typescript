...
<!-- UsageSnippet language="typescript" operationID="accounting.companiesAll" method="get" path="/accounting/companies" -->
```typescript
...
import { accountingCompaniesList } from "@apideck/unify/funcs/accountingCompaniesList.js";
...
async function run() {
  const res = await accountingCompaniesList(apideck, {
    serviceId: "salesforce",
    companyId: "12345",
    fields: "id,updated_at",
    passThrough: {
  ...
  if (res.ok) {
    ...
    console.log("accountingCompaniesList failed:", res.error);
  }
}
...
**Promise\<[operations.AccountingCompaniesAllResponse](../../models/operations/accountingcompaniesallresponse.md)\>**
