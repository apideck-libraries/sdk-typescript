...
export function invoicesSortFromJSON(
  jsonString: string,
): SafeParseResult<InvoicesSort, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => InvoicesSort$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'InvoicesSort' from JSON`,
  );
}
